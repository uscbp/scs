...
public class _Element extends NslClass{
...
public void initElement(int max) 
{
	do 
	{
		x = (int)NslRandom.eval()%max;
	 	y = (int)NslRandom.eval()%max;
		//  Make sure Element location is not the fovea
	} while (x==FOVEAX&&y==FOVEAY);

	do 
	{
		xo = (int)NslRandom.eval()%max;
		yo = (int)NslRandom.eval()%max;
...
{
	_Element cur, last;
	_Element temp = new _Element("temp", this);
...
{
	_Element first = new _Element("first", this);
          // <- Check null constructor
...
/*
            ...
            tyo = nslj.src.system.Console.readInt("Enter y coordinates for second target (range:0-"
                  +(CorticalArraySize-1)+"): ");
*/
		if ( (tx<0)||(tx>CorticalArraySize-1)||(ty<0)||(ty>CorticalArraySize-1)||(txo<0)||(txo>CorticalArraySize-1)||(tyo<0)||(tyo>CorticalArraySize-1) ) 
		{
			system.nslPrintln("Coordinates out of range.  Try again.");
		}
		else 
		{
...
public _Element GetElement( ) 
{
	...
	{
		counter++;
		system.nslPrintln(counter+","+cur.X()+"  "+cur.Y()+"  "+cur.XO()+"  "+cur.YO() );
		cur = cur.Next();
	}
...
public void makeConn(){}
	...
	/* GENERIC CONSTRUCTOR: */
	public _Element(String nslName, NslModule nslParent)
{
		super(nslName, nslParent);
		makeInst_Element(nslName, nslParent);
	}


	public _Element(String nslName, NslClass nslParent) {
		super(nslName, nslParent);
		makeInst_Element(nslName, nslParent);
	}

	public void makeInst_Element(String nslName, NslModule nslParent)
{ 
	...
	public void makeInst_Element(String nslName, NslClass nslParent) { 
		Object[] nslArgs=new Object[]{};
		callFromConstructorTop(nslArgs);
		callFromConstructorBottom();
	}
