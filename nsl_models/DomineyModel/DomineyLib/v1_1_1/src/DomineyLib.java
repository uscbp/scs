...
public class DomineyLib extends NslClass{
...
//--------------------------------------------------------------------
    ...
    */
        ...
        // int yPlusStdsz=0;
         ...
         float[][] retina;  //stdsz
         NslFloat2 nretina = new NslFloat2(stdsz, stdsz);
    ...
    // and i=0, j=0 is in the upper lefthand corner.
        for (i = 0; i<=retinay ; i++) {
            ...
            //aa: 99/7/18 changed bounds check from 2.1.7 version
                if  ( (iMinusYPlusStdsz<=bigszMinus1)&&(iMinusYPlusStdsz>=0)&&(jPlusXPlusStdsz<=bigszMinus1)&&(jPlusXPlusStdsz>=0)){
    ...
    {
        NslFloat2 nwinner = new NslFloat2("nwinner", stdsz, stdsz);
        ...
        for (i = 0; i<=inputx ; i++)
            for (j = 0; j<=inputy ; j++)
                if (__tempDomineyLib0.setReference(ninput.get(i, j)).get()>thresh )
                {
                    thresh = (float)__tempDomineyLib1.setReference(ninput.get(i, j)).get();
                     threshi = i;
                     threshj = j;
                }
        if (thresh>threshold )
            nwinner.set(threshi, threshj,   thresh);
        return nwinner;
    }
    //--------------------------------------------------------------------
    public NslFloat2 shift(NslFloat0 nxxx, NslFloat0 nyyy,NslFloat0 nkkk,int stdsz)
    {
        ...
        NslFloat2 nmask = new NslFloat2("nmask", stdsz, stdsz);
...
public void makeConn(){}
	...
	/* Temporary variables */
		NslFloat0 __tempDomineyLib0 = new NslFloat0();
		NslFloat0 __tempDomineyLib1 = new NslFloat0();

	/* GENERIC CONSTRUCTOR: */
	public DomineyLib(String nslName, NslModule nslParent)
{
		super(nslName, nslParent);
		makeInstDomineyLib(nslName, nslParent);
	}


	public DomineyLib(String nslName, NslClass nslParent) {
		super(nslName, nslParent);
		makeInstDomineyLib(nslName, nslParent);
	}

	public void makeInstDomineyLib(String nslName, NslModule nslParent)
{ 
	...
	public void makeInstDomineyLib(String nslName, NslClass nslParent) { 
		Object[] nslArgs=new Object[]{};
		callFromConstructorTop(nslArgs);
		callFromConstructorBottom();
	}
