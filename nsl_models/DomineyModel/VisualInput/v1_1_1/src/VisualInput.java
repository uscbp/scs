...
public class VisualInput extends NslModule{
...
//variables 
public  NslDoutFloat2 visualinput; // 
public  NslDoutFloat2 visualinputSub; // 
...
{	
	...
	if (system.debug>1) 
	{
		system.nslPrintln("VisualInput:initRun:The protocol num is: "+protocolNum.toString());
	}
	...
	visualinput.set(center, center, value);
}
...
public void simRun() 
{
	...
	if (system.debug>=1) 
	{
		system.nslPrintln("VisualInput:simRun: NSLJ currentTime: "+currentTime);
		system.nslPrintln("VisualInput:simRun: NSLJ currentTimePlusDelta: "+currentTimePlusDelta);
        }
	currentTime=currentTimePlusDelta;

	// all non-manual protocolNums have fixation on from 0 to 0.02
	if ((protocolNum.get()!=0)&&(currentTime>=0.00)&&(currentTime<=0.02))
	{
		visualinput.set(center, center, value);
	}
	// ("single")
	if (protocolNum.get()==1) 
	{
		// fixation - off
		if (currentTime>0.02)
		{
			visualinput.set(center, center, 0);
		}
		// light up first target
		if ((currentTime>=0.02)&&(currentTime<=0.07)) 
		{
			visualinput.set(centerP3, centerM3, value);
//			visualinput[center][centerP3]=value;
		}
		//  turn off target
		if (currentTime>0.07) 
		{
			visualinput.set(centerP3, centerM3, 0);
//			visualinput[center][centerP3]=0;
		}
	}
	// ("memorySingleI"))
	else if (protocolNum.get()==2) 
	{
		...
		if ((currentTime>=0.02)&&(currentTime<=0.07))
		{
			visualinput.set(centerM2, centerP2, value);
		}
		// unlight first target
		if (currentTime>0.07)
		{
			visualinput.set(centerM2, centerP2, 0);
		}
	}
	// ("memorySingleII"))
	else if (protocolNum.get()==3) 
	{
		// was .58 but that delay was not needed
		if (currentTime>0.28)
		{
		...
		if ((currentTime>=0.02)&&(currentTime<=0.07))
		{
			visualinput.set(centerM2, centerM3, value);
		}
		// unlight first target
		if (currentTime>0.07)
		{
			visualinput.set(centerM2, centerM3, 0);
		}
	}
	//("doubleI")
	else if (protocolNum.get()==4) 
	{
		if (currentTime>0.02)
		{
			visualinput.set(center, center, 0);
		...
		if ((currentTime>=0.02)&&(currentTime<=0.07))
		{
			visualinput.set(centerM3, center, value);
		}
		// unlight first target
		if ((currentTime>0.07)&&(currentTime<=0.08)) 
		{
			visualinput.set(centerM3, center, 0);
		}
		// light up second target
		if ((currentTime>=0.09)&&(currentTime<=0.13)) 
		{
			visualinput.set(centerM3, centerP3, value);
		}
		// unlight second target
		if (currentTime>0.13) 
		{
			visualinput.set(centerM3, centerP3, 0);
		}

	} 
	//("doubleII")
	else if (protocolNum.get()==5) 
	{
		if (currentTime>0.02)
		{
			visualinput.set(center, center, 0);
		...
		if ((currentTime>=0.02)&&(currentTime<=0.07))
		{
			visualinput.set(center, centerM2, value);
		}
		// unlight first target
		if ((currentTime>0.07)&&(currentTime<=0.08)) 
		{
			visualinput.set(center, centerM2, 0);
		}
		// light up second target
	...
	{
		system.nslPrintln("That protocolNum is not recognized: protocolNum: "+protocolNum.toString());
		//nslPrintln("That protocolNum is not recognized: protocolNum: ",protocolNum);
	}

	// for debug
	NslGetSector.eval(visualinputSub.get(),visualinput,bigszDiv3,bigszDiv3,bigszDiv3Time2Min1,bigszDiv3Time2Min1);
	if (system.debug>=1) 
	{
		system.nslPrintln("VisualInput:visualinputSub");
		system.nslPrintln(visualinputSub);
	}
} //end simRun
public void makeConn(){
	...
	/* GENERIC CONSTRUCTOR: */
	public VisualInput(String nslName, NslModule nslParent, int stdsz, int bigsz)
{
		super(nslName, nslParent);
		this.stdsz=stdsz;
		this.bigsz=bigsz;
		initSys();
		makeInstVisualInput(nslName, nslParent, stdsz, bigsz);
	}

	public void makeInstVisualInput(String nslName, NslModule nslParent, int stdsz, int bigsz)
{ 
		Object[] nslArgs=new Object[]{stdsz, bigsz};
		callFromConstructorTop(nslArgs);
		visualinput = new NslDoutFloat2("visualinput", this, bigsz, bigsz);
		visualinputSub = new NslDoutFloat2("visualinputSub", this, stdsz, stdsz);
